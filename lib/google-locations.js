...
const GoogleLocations = function(key, options) {
  // Copy options object
  let options = _.extend({}, options);
  // Set default options
  if (!options) options = {};
...
GoogleLocations.prototype.searchByAddress = function(config, cb) {
  ...
  let _self = this;
  let options = _generateOptions(config);
  _self.geocodeAddress(options.geocode, function(err, result){
    ...
    try {
      ...
      let query = _searchQuery(location, options.search);
...
function _batchDetails(context, result, maxResults, cb) {
  ...
  let details = [];
  let errors = [];
  let j = (maxResults > result.results.length) ? result.results.length : maxResults;
  for (let i = 0; i < j; i++) {
    try {
...
function _generateOptions(geocodeOptions) {
  // Copy options object
  let geocodeOptions = _.extend({}, geocodeOptions);
  let options = {geocode: geocodeOptions, search: {maxResults: 1, rankby: "distance"}};
  const keys = ['name', 'maxResults', 'rankby', 'radius'];
...
function _searchQuery(location, searchOptions) {
  // Copy options object
  let searchOptions = _.extend({}, searchOptions);
  let query = {location: [location.lat, location.lng], rankby: searchOptions.rankby};
...
function _generateUrl(context, query, type, method) {
  ...
  let request_query = url.parse(url.format({
    protocol: 'https',
